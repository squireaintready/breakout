...
export default function TradeJournal() {
  ...
  const stats = useMemo(() => {
    ...
    return { winRate, avgWin, avgLoss, avgRR, profitFactor, total: trades.length, totalFees };
  }, [trades]);
      ...
      <div className="flex gap-2 flex-wrap">
        {(['all', 'win', 'loss'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1 rounded text-sm ${filter === f ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-400'}`}>
            ...
            <div key={trade.id} className="bg-slate-800 rounded-lg p-3 text-sm">
              ...
              {editingId === trade.id ? (
                <div className="mt-2 flex gap-1">
                  ...
                  <button onClick={() => { editTrade(trade.id, { notes: editNotes }); setEditingId(null); }}
                ...
                <div className="mt-1 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setEditingId(trade.id); setEditNotes(trade.notes); }}
